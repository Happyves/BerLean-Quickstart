...
lemma fermat_stricly_monotone {n : ℕ} :
  ...
  rw [Nat.pow_succ]
  apply (pow_lt_pow_iff_right one_lt_two).mpr
...
lemma fermat_odd {n : ℕ} :
  ...
  constructor
  · exact even_two
  · exact Nat.ne_of_gt (pow_pos zero_lt_two _)
...
lemma second_proof_standardised :
  ...
  by
  -- We define a function obtained from the axiom of choice, applied to `second_proof`
  -- It is the `P` of `second_proof`
  set f := Classical.choose second_proof with f_def
  -- This is the property of `P` in `second_proof`
  have f_prop := Classical.choose_spec second_proof
  rw [← f_def] at f_prop
  -- We'll show that the set is infinite by injecting ℕ in it
